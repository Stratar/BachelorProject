...
class OsimModel(object):
    ...
    def __init__(self, model_path, k_paths_dict, visualize, integrator_accuracy = 1e-3): # Reduced accuracy for greater speed
        ...
        model_path = os.path.normpath(os.path.join(self.osim_path, model_path))
...
class ProstheticsEnvMulticlip(OsimEnv):
    ...
    #model_path = os.path.join(os.path.dirname(__file__), '../models/healthy_subject_model_customized') 
    ...
    def compute_reward(self, t, kin_df):
        ...
        ankle_loss = (state_desc['joint_pos']['ankle_l'] - self.kins['ankle_angle_l'][t])**2 + (state_desc['joint_pos']['ankle_r'] - self.kins['ankle_angle_r'][t])**2
        ...
        ankle_loss_v = (state_desc['joint_vel']['ankle_l'] - self.kins['ankle_angle_l_speed'][t])**2  + (state_desc['joint_vel']['ankle_r'] - self.kins['ankle_angle_r_speed'][t])**2 
    ...
    def step(self, action, project = True):
        ...
        return [ obs, reward, penalty, self.is_done() or (self.osim_model.istep >= (self.spec.timestep_limit+self.osim_model.start_point))]
